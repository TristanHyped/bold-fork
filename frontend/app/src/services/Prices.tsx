...
import { CustomTokenSymbols, GOVERNANCE_COIN_SYMBOL, isCollateralSymbol } from "@liquity2/uikit";
import { useQuery } from "@tanstack/react-query";
import * as dn from "dnum";
// import * as v from "valibot";
import { useReadContract } from "wagmi";

type PriceToken = CustomTokenSymbols | CollateralSymbol;
...
//   });
// }
// @todo: add support for our prices
export function usePrice<PT extends PriceToken>(symbol: PT | null): UseQueryResult<Dnum> {
  ...
  const fromPriceFeed = symbol !== null && isCollateralSymbol(symbol);
  ...
  const boldPrice = useQuery({
    ...
    enabled: symbol === GOVERNANCE_COIN_SYMBOL,
  });
  ...
  if (symbol === GOVERNANCE_COIN_SYMBOL) {
    return boldPrice;
  }
